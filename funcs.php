<?php
	...
	function activarUsuario($id)
	{
		...
		$stmt = $mysqli->prepare("UPDATE usuarios SET estado= 'activo' WHERE id_usuario = ?");
	...
	function isActivo($usuario)
	{
		...
		$stmt = $mysqli->prepare("SELECT estado FROM usuarios WHERE  correo = ? LIMIT 1");
		$stmt->bind_param('s', $usuario);
		$stmt->execute();
		$stmt->bind_result($activacion);
		$stmt->fetch();
	...
	function generaTokenPass($user_id)
	{
		...
		$token = generateToken();
		//print($token);
		$stmt = $mysqli->prepare("UPDATE usuarios SET token_pass=?, pass_request=1 WHERE id_usuario = ?");
	...
	function getPasswordRequest($id)
	{
		...
		$stmt = $mysqli->prepare("SELECT password_request FROM correo WHERE id_usuario = ?");
	...
	function verificaTokenPass($user_id, $token){
		...
		$stmt = $mysqli->prepare("SELECT estado FROM usuarios WHERE id_usuario = ? AND token_pass = ? AND pass_request = 1 LIMIT 1");
	...
	function cambiaPassword($password, $user_id, $token){
		...
		$stmt = $mysqli->prepare("UPDATE usuarios SET contrasena = ?, token_pass='', pass_request=0 WHERE id_usuario = ? AND token_pass = ?");
